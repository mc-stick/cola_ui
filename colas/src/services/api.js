...
class API {
  ...
  async verificarUsuario(username) {
    try {
      ...
      if (!result.ok) {
        //console.warn(result.message); // mensaje de error del backend
        return null;
      }
  ...
  async getTodosPermisosUsuarios() {
    try {
      const response = await fetch(`${API_URL}/usuarios/permisos/todos`);
  ...
  async getOperadorServicios(usuarioId) {
    try {
      const response = await fetch(
        `${API_URL}/operadores/${usuarioId}/servicios`,
      );
      if (!response.ok) {
  ...
  async asignarServicioOperador(usuarioId, servicioId) {
    try {
      const response = await fetch(
        `${API_URL}/operadores/${usuarioId}/servicios/${servicioId}`,
        {
          method: "POST",
          headers: this.getAuthHeaders(),
        },
  ...
  async GetTicketEvaluationState(ticketId) {
    try {
      const response = await fetch(
        `${API_URL}/tickets/${ticketId}/estado-evaluacion`,
        {
          headers: this.getAuthHeaders(),
        },
      );
  ...
  async getEstadisticasServicios(fechaInicio, fechaFin) {
    try {
      ...
      const response = await fetch(
        `${API_URL}/estadisticas/servicios?${params}`,
      );
      if (!response.ok) {
  ...
  async getEstadisticasOperadores(fechaInicio, fechaFin) {
    try {
      ...
      const response = await fetch(
        `${API_URL}/estadisticas/operadores?${params}`,
      );
      if (!response.ok) {
  ...
  async getEstadisticasHoras(fecha) {
    try {
      const queryString = fecha ? `?fecha=${fecha}` : "";
      const response = await fetch(
        `${API_URL}/estadisticas/horas${queryString}`,
      );
      if (!response.ok) {
  ...
  async PrintTicket(ticket, servicio) {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error: result.error || "Error al imprimir ticket",
        };
      }
  ...
  async getCurrentUser() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          success: false,
          error: data.error || "Error obteniendo usuario",
