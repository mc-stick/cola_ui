...
router.get('/operadores', async (req, res) => {
  try {
    ...
    const query = `
      SELECT 
        u.id, u.username, p.numero AS puesto_numero,
        COUNT(t.id) AS total_tickets,
        SUM(CASE WHEN t.estado = 4 THEN 1 ELSE 0 END) AS atendidos,
        SUM(CASE WHEN t.estado = 5 THEN 1 ELSE 0 END) AS no_presentados,
        AVG(
      ...
      LEFT JOIN tickets t 
        ...
        AND DATE(t.created_at) >= ? AND DATE(t.finalizado_at) <= ?
      LEFT JOIN (
        SELECT 
          t.usuario_id,
      ...
      GROUP BY u.id, u.username, p.numero, ev.evaluados, ev.promedio_evaluacion
      ORDER BY total_tickets DESC
    `;
